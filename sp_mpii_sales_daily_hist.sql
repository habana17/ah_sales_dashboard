create
or replace PROCEDURE sp_mpii_sales_daily_hist
AS 
...
NAME:       sp_mpii_sales_daily_hist
PURPOSE:    travel & ah transactions for mpii

REVISIONS:
Ver          Date                  Author             Description
---------  ----------          ---------------  ------------------------------------
1.0        10/14/2025            Francis          1. sp_mpii_sales_daily_hist
...
---------------------------------------------------ARCHIVE & DELETE--------------------------------------------------------
        ...
        INSERT INTO MPII_IRISNLR_TRANSACTIONS_DAILY_HIST
        SELECT 
            d.*,
            SYSDATE as archive_date
        FROM MPII_IRISNLR_TRANSACTIONS_DAILY d
        WHERE 1=1
        AND TRUNC(trandate) = p_date;

        -- Delete data from source table
        DELETE FROM MPII_IRISNLR_TRANSACTIONS_DAILY
        WHERE 1=1
        AND TRUNC(trandate) = p_date;  
...
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_RAW_PORTFOLIO_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','DELETE');
DELETE 
FROM 
adw_prod_tgt.TEMP_MPII_RAW_PORTFOLIO_DAILY;

adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_RAW_PORTFOLIO_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','INSERT');
...
--             )
;
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_RAW_PORTFOLIO_DAILY','sp_mpii_sales_daily_hist',SYSDATE,SYSDATE,'UPDATE');


-- Step 2: Insert data to TEMP_MPII_NLR_PORTFOLIO_PISC_EIS_DAILY
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_NLR_PORTFOLIO_PISC_EIS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','DELETE');
DELETE 
FROM 
adw_prod_tgt.TEMP_MPII_NLR_PORTFOLIO_PISC_EIS_DAILY;

adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_NLR_PORTFOLIO_PISC_EIS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','INSERT');
...
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_NLR_PORTFOLIO_PISC_EIS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,SYSDATE,'UPDATE');


adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_IRISNLR_TRANSACTIONS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','DELETE');
DELETE 
FROM 
adw_prod_tgt.TEMP_MPII_IRISNLR_TRANSACTIONS_DAILY;

-- Step 3: Insert data to TEMP_MPII_IRISNLR_TRANSACTIONS_DAILY 
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_IRISNLR_TRANSACTIONS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','INSERT');
...
--       fngetrefdesc (platform),
 ...
 WHERE     a.polno = b.polno
    ...
    --    and a.trandate >= DATE '2025-07-01' AND a.trandate < DATE '2025-08-01' --incremental
       ...
       ;
adw_prod_tgt.sp_adw_table_logs('TEMP_MPII_IRISNLR_TRANSACTIONS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,SYSDATE,'UPDATE');


-- Step 4: Insert data to MPII_IRISNLR_TRANSACTIONS_DAILY -- final insert 

adw_prod_tgt.sp_adw_table_logs('MPII_IRISNLR_TRANSACTIONS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,'','INSERT');
...
/*******old 1 *******/
    ...
    adw_prod_tgt.sp_adw_table_logs('MPII_IRISNLR_TRANSACTIONS_DAILY','sp_mpii_sales_daily_hist',SYSDATE,SYSDATE,'UPDATE');
...
EXCEPTION
    ...
    BEGIN        
        ...
        VALUES ('sp_mpii_sales_daily_hist', err_msg, 'Main procedure failed');
        COMMIT;
    END;


END sp_mpii_sales_daily_hist;