CREATE OR REPLACE PROCEDURE sp_ah_missing_ac
AS

/******************************************************************************

NAME:       sp_ah_missing_ac
PURPOSE:    

REVISIONS:
Ver          Date                  Author             Description
---------  ----------          ---------------  ------------------------------------
1.0        02/09/2026           Francis              1. Create sp_ah_missing_ac
...
BEGIN 
    ...
    --INSERT ah data 
    BEGIN
                DELETE FROM TEMP_EOM_BILL_DAILY_MIS;

                INSERT INTO adw_prod_tgt.TEMP_EOM_BILL_DAILY_MIS
                (
                    ...
                    batchno in  (
                                ...
                                WHERE 1=1 
                                AND line_pref = 'AC'
                                AND eis_flag IS NULL
                                AND prod_flag IS NULL
                                )
                    ;  
                    commit;                          
    END;
    ...
    BEGIN
                    DELETE FROM TEMP_ISRHOS_DAILY_MIS;

                    INSERT INTO adw_prod_tgt.TEMP_ISRHOS_DAILY_MIS
                    (
                    ...
                    SELECT DISTINCT
                        ...
                        a.billeffdte AS billbilleffdate, a.duedate AS billduedate, a.old_grossprem, a.new_grossprem, 
                        a.oldbill_no, a.dfixtag, a.refno, a.billseqno, a.billstat, a.canceldate, a.remarks,
                        a.lifecnt, a.paymode, a.issuedate, a.billeffdte, a.duedate, a.netprem, a.premtax, a.lgt, a.docstamps, 
                        a.adminfee, a.othercharges, a.commamt, a.commwtax, a.sfeeamt, a.sfeetax, a.totamtdue, a.userid,
                        a.timestmp, a.polno, a.polyer, a.osbalance, a.commwtaxamt, a.sfeewtaxamt, a.subsidiary, a.batchno,
                        a.reinstag, a.totsi, a.discounted_amt, a.discount_value, a.discount_rate, a.promocode, a.forex, a.vatamt,
                        a.vatrate, a.fullbill, a.appl_no, a.commrate, a.sfeerate
    ...
    BEGIN
                        ...
                        INSERT INTO adw_prod_tgt.TEMP_EOM_AHR_CANCEL_DAILY_MIS
                        (
                            BATCHNO,
                            POLNO
                        )
                        ...
                        WHERE f.enddate IS NULL 
                            ...
                            d.prodcode, d.statcode, h.billmode, g.dist, 
                            f.subline, f.LOB, d.statcode, d.policytype, f.prodcategory, 
                            d.segmentcode, d.branchcode, f.co_code, d.currency, 
                            f.orgtype, f.mainline, g.nameid, g.pertype, b.trantype, d.issue_source,              
    ...
    BEGIN
                        ...
                        INSERT INTO adw_prod_tgt.TEMP_NLR_PORTFOLIO_PISC_DAILY_MIS --insert the first data   
                        (
                ...
                FROM adw_prod_tgt.nlr_policy_tran_v2 b
                   ...
                   LEFT JOIN adw_prod_tgt.TEMP_EOM_BILL_DAILY_MIS tb ON tb.batchno = b.batchno     
                   LEFT JOIN adw_prod_tgt.TEMP_ISRHOS_DAILY_MIS ti ON ti.batchno = b.batchno     
                   JOIN adw_prod_tgt.nlr_policy_mst_v2 d ON b.polno = d.polno
                   LEFT JOIN adw_prod_tgt.gct_geninfo_ref dc 
                   ON dc.refseqno = d.pol_source --polsource
                ...
                INSERT INTO TEMP_NLR_PORTFOLIO_PISC_DAILY_MIS --insert the second data  
                    (
                    batchno,polno,prodline,polyear,serv_agentcode,
                    serv_agentname,segment_code,cardclient,clntid,
                    policyholder,poltrneffdate,effdate,expdate,trandate,branchcode,
                ...
                and b.polno in (select polno from adw_prod_tgt.TEMP_EOM_AHR_CANCEL_DAILY_MIS)
                AND b.batchno in 
                (
                    SELECT batchno 
                    FROM temp_missing_batchno
